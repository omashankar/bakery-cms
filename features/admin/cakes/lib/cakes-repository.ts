...
import type { Cake, CakeFormData } from "@/types";
import {
  adminCategories,
  adminFlavours,
  adminOccasions,
...
import { DEFAULT_CAKE_SHAPES } from "./cake-mapper";
import { slugify } from "./cake-utils";
...
const CAKES_STORAGE_VERSION = 5;

function nowIso(): string {
  return new Date().toISOString();
}

function mapLandingCakeToAdmin(cake: LandingCake, index: number): Cake {
...
function getSeedLandingCakes(): LandingCake[] {
  const combined = [
    ...featuredCakes,
    ...trendingCakes,
    ...bestSellers,
...
function seedCakes(): Cake[] {
  return getSeedLandingCakes().map(mapLandingCakeToAdmin);
}

function normalizeCommerceFields(cake: Cake): Cake {
  ...
  return {
    ...
    shapes: cake.shapes?.length ? cake.shapes : [...DEFAULT_CAKE_SHAPES],
...
function normalizeCakeImages(cakes: Cake[]): { cakes: Cake[]; changed: boolean } {
  let changed = false;
...
export function loadCakes(): Cake[] {
  if (typeof window === "undefined") return seedCakes();

  const raw = localStorage.getItem(STORAGE_KEY);
  if (!raw) {
    const seeded = seedCakes();
    persistCakes(seeded);
  ...
  try {
    const parsed = JSON.parse(raw) as Cake[];
    ...
    const { cakes: normalized, changed } = normalizeCakeImages(parsed);

    if (changed || storedVersion < CAKES_STORAGE_VERSION) {
      persistCakes(normalized);
      localStorage.setItem(STORAGE_VERSION_KEY, String(CAKES_STORAGE_VERSION));
    }
...
function persistCakes(cakes: Cake[]): void {
  if (typeof window === "undefined") return;
  localStorage.setItem(STORAGE_KEY, JSON.stringify(cakes));
}

export function getCakeById(id: string): Cake | null {
  return loadCakes().find((cake) => cake.id === id) ?? null;
}

export function getCakeBySlug(slug: string): Cake | null {
  return loadCakes().find((cake) => cake.slug === slug) ?? null;
}

export function createCake(data: CakeFormData): Cake {
  ...
  persistCakes([cake, ...cakes]);
  return cake;
}

export function updateCake(id: string, data: CakeFormData): Cake | null {
  const cakes = loadCakes();
...
export function bulkUpdateStatus(ids: string[], status: Cake["status"]): number {
  const cakes = loadCakes();
  let count = 0;
  ...
  persistCakes(updated);
  return count;
}

export function bulkDeleteCakes(ids: string[]): number {
  const cakes = loadCakes();
...
export function createEmptyCakeForm(): CakeFormData {
  return {
    name: "",
    slug: "",
    description: "",
